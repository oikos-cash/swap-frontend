  ...
  const getBalance = (address, tokenAddress) => {
    ...
    if (!tokenAddress || tokenAddress === 'TRX') {
      ...
      if (!balance) {
        ...
        return Balance(0, 'TRX');
      }
      return balance;
    //} else if (tokenAddress === '41d20a4b6d1c8ea0c1aee0631870e31fd99037dfda') {
...
export const initialize = () => (dispatch, getState) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (typeof window.tronWeb !== 'undefined' && !!window.tronWeb.defaultAddress.hex) {
 
        web3 = window.tronWeb; //new Web3(window.ethereum);
      ...
      console.error('User denied access or Tronlink not installed. ');
...
export const sync = () => async (dispatch, getState) => {
  ...
  // Sync Account
  const accounts = await web3.trx.getAccount(web3.defaultAddress.hex) //web3.eth.getAccounts();
  //console.log('got accts', accounts);
 
  if (account !== accounts.address) {
    dispatch({ type: UPDATE_ACCOUNT, payload: accounts.address});
    dispatch(watchBalance({ balanceOf: accounts.address }));
  }
  ...
  if (!networkId) {
    dispatch({
      ...
      //HARDCODED
      payload: 1,//await web3.eth.net.getId(),
    });
  }

  // Sync Ethereum Balances
  watched.balances.ethereum.forEach(async address => {
    const balance = await web3.trx.getBalance(address);
    //const { value } = getBalance(address);
    const value = BN( (balance));
    //console.log("address", web3.address.fromHex(address), "got balance", value)

    //if (value.isEqualTo(BN(trx2Wei))) {
    //  return;
    //}
  ...
  Object.keys(watched.balances)
    .forEach(async tokenAddress => {
      ...
      const contract = contracts[tokenAddress] || await web3.contract().at(tokenAddress); //new web3.eth.Contract(ERC20_ABI, tokenAddress);
      const contractBytes32 = contract ;// contracts[tokenAddress] || new web3.eth.Contract(ERC20_WITH_BYTES_ABI, tokenAddress);
      ...
      watchlist.forEach(async address => {
        ...
        let o = await contract.balanceOf(address).call();

        if (!o.balance) o = { balance: o }
        //console.log("contract address", web3.address.fromHex(contract.address), "balance",  o );
        ...
        try {
          ...
          try {
            symbol = symbol || web3.toUtf8(await contractBytes32.methods.symbol().call().catch());
          } catch (err) {
          }
        }

        if (tokenBalance.value.isEqualTo(BN(o.balance)) && tokenBalance.label && tokenBalance.decimals) {
          return;
        }
        ...
        dispatch({
          ...
          payload: {
            tokenAddress,
            balanceOf: address,
            balance: Balance(o.balance, symbol, decimals),
          },
        });
      });
    });
  ...
  Object.entries(watched.approvals)
    .forEach(async ([tokenAddress, token]) => {
      const contract = contracts[tokenAddress] || await web3.contract().at(tokenAddress); //new web3.eth.Contract(ERC20_ABI, tokenAddress);
      const contractBytes32 = contract ; //contracts[tokenAddress] || new web3.eth.Contract(ERC20_WITH_BYTES_ABI, tokenAddress);
      ...
      Object.entries(token)
        .forEach(([ tokenOwnerAddress, tokenOwner ]) => {
          tokenOwner.forEach(async spenderAddress => {
            ...
            const balance = await contract.allowance(tokenOwnerAddress, spenderAddress).call();
            const decimals = approvalBalance.decimals || await contract.decimals().call();
            
            let symbol = approvalBalance.label;
            try {
              symbol = symbol || await contract.symbol().call();
            } catch (e) {
              try {
                symbol = symbol || web3.toUtf8(await contractBytes32.symbol().call());
              } catch (err) {
              }
            }
...
const checkTx = (txId) => {
      return web3.trx.getTransaction(txId).then(r => {
