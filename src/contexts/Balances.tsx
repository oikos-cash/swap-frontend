  ...
  const allExchanges = useMemo(
    () =>
      Object.keys(allTokenDetails)
        .filter(tokenAddress => tokenAddress !== 'TRX')
        .map(tokenAddress => ({
          tokenAddress,
      ...
      Promise.all(
        allExchanges
          .filter(({ exchangeAddress, tokenAddress }) => {
            ...
            const hasValueETH = !!stateRef.current?.[chainId]?.[exchangeAddress]?.['TRX']?.value

            const cachedFetchedAsOfToken = fetchedAsOfCache.current?.[chainId]?.[exchangeAddress]?.[tokenAddress]
            const cachedFetchedAsOfETH = fetchedAsOfCache.current?.[chainId]?.[exchangeAddress]?.['TRX']
            ...
            const fetchedAsOfETH =
              stateRef.current?.[chainId]?.[exchangeAddress]?.['TRX']?.blockNumber ?? cachedFetchedAsOfETH
          ...
          .map(async ({ exchangeAddress, tokenAddress }) => {
            ...
            return Promise.all([
              ...
              fetchBalance(exchangeAddress, 'TRX')
        ...
        batchUpdateExchanges(
          ...
          results.flatMap(result => [result.tokenAddress, 'TRX']),
...
export function useExchangeReserves(tokenAddress: string) {
  ...
  const reserveETH = useAddressBalance(exchangeAddress, 'TRX')
...
export function useETHPriceInUSD_old() {
  const { chainId } = useWeb3React()
...
export function useETHPriceInUSD() {
  ...
  const [price, setPrice] = useState<undefined | null | object>()
  useEffect(() => {
    const getTRXPriceUSD = async () => {
    ...
    const updatePrice = async () => {
      const price = await getTRXPriceUSD()
      setPrice(price)
    }
    updatePrice()
