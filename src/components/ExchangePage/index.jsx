...
function getSwapType(inputCurrency, outputCurrency) {
  if (!inputCurrency || !outputCurrency) {
    return null
  } else if (inputCurrency === 'TRX') {
    return ETH_TO_TOKEN
  } else if (outputCurrency === 'TRX') {
    return TOKEN_TO_ETH
  } else {
...
function getInitialSwapState(state) {
  return {
    ...
    inputCurrency: state.inputCurrencyURL ? state.inputCurrencyURL : 'TRX',
    outputCurrency: state.outputCurrencyURL
      ? state.outputCurrencyURL === 'TRX'
        ? state.inputCurrencyURL && state.inputCurrencyURL !== 'TRX'
          ? 'TRX'
          : ''
...
function getMarketRate(
  ...
  if (swapType === ETH_TO_TOKEN) {
    return getExchangeRate(outputReserveETH, 6, outputReserveToken, outputDecimals, invert)
  } else if (swapType === TOKEN_TO_ETH) {
    return getExchangeRate(inputReserveToken, inputDecimals, inputReserveETH, 6, invert)
  } else if (swapType === TOKEN_TO_TOKEN) {
    const factor = ethers.utils.bigNumberify(10).pow(ethers.utils.bigNumberify(6))
    const firstRate = getExchangeRate(inputReserveToken, inputDecimals, inputReserveETH, 6)
    const secondRate = getExchangeRate(outputReserveETH, 6, outputReserveToken, outputDecimals)
    try {
...
export default function ExchangePage({ initialCurrency, sending = false, params }) {
  ...
  useEffect(() => {
    ...
    if (inputBalance && (inputAllowance || inputCurrency === 'TRX') && inputValueCalculation) {
      if (inputBalance.lt(inputValueCalculation)) {
        setInputError(t('insufficientBalance'))
      } else if (inputCurrency !== 'TRX' && inputAllowance.lt(inputValueCalculation)) {
  ...
  const percentSlippage =
    exchangeRate && marketRate && !marketRate.isZero()
      ? exchangeRate
          .sub(marketRate)
          .abs()
          .mul(ethers.utils.bigNumberify(10).pow(ethers.utils.bigNumberify(6)))
          .div(marketRate)
  ...
  const newInputDetected =
    inputCurrency !== 'TRX' && inputCurrency && !INITIAL_TOKENS_CONTEXT[chainId].hasOwnProperty(inputCurrency)
      ...
      <CurrencyInputPanel
        ...
        extraTextClickHander={() => {
          if (inputBalance && inputDecimals) {
            const valueToSet = inputCurrency === 'TRX' ? inputBalance.sub(ethers.utils.parseEther('.1')) : inputBalance
