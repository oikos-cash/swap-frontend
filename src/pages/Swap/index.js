...
class Swap extends Component {
  ...
  validate() {
    ...
    //console.log("inputBalance", inputBalance, "inputValue", inputValue, "inputDecimals", inputDecimals);

    //if (inputBalance.isLessThan( BN(inputValue * 10 ** _inputDecimals) )) {
    //console.log("condition", BN(inputBalance).lt(BN(inputValue * 10 ** inputDecimals) ));

    if ( BN(inputBalance).lt(BN(inputValue * 10 ** inputDecimals))   ) {
  ...
  isUnapproved() {
    ...
    if (!inputCurrency || inputCurrency === 'TRX') {
      return false;
    }
  ...
  recalcForm() {
    ...
    if (inputCurrency !== 'TRX' && outputCurrency !== 'TRX') {
  ...
  recalcTokenTokenForm = () => {
    ...
    const { value: outputReserveA }= selectors().getBalance(exchangeAddressA, 'TRX');
    const { value: inputReserveB } = selectors().getBalance(exchangeAddressB, 'TRX');
  ...
  recalcEthTokenForm = () => {
    ...
    const tokenAddress = [inputCurrency, outputCurrency].filter(currency => currency !== 'TRX')[0];
  ...
  onSwap = async () => {
    ...
    const exchanges = { 'TBFdp2jfMccwneRnbcobCa6txrxo1a1tvi':'TQWkJqpnx9iuanoTqYMiPyc6PDVmU8HUAZ', 'TNEzrZcNHy7jkEhFaNgeff6GHbZe329uVC':'THQbsSwQdR4NgzAzoucrMeiWgChHBfBGzJ'}
    let deadline;
    try {
    ...
    if (lastEditedField === INPUT) {
      ...
      switch(type) {
        case 'TRX_TO_TOKEN':
          ...
          //web3.contract().at( fromToken[outputCurrency] ).then(c=>{
            const c = await web3.contract().at(fromToken[outputCurrency]);
            console.log("got c", c);

            //console.log("got c", c);
            c.TrxToTokenSwapInput(
              BN(outputValue).multipliedBy(10 ** outputDecimals).multipliedBy(1 - ALLOWED_SLIPPAGE).toFixed(0),
              deadline*1000,
            )
            .send({
              from: account,
              callValue:  web3.toSun(inputValue), // BN(inputValue).multipliedBy(10 ** 6).toFixed(0),
            }, (err, data) => {
              if (!err) {
                addPendingTx(data);
                this.reset();
        ...
        case 'TOKEN_TO_TRX':
         ...
         web3.contract().at(fromToken[inputCurrency] ).then( c => {
           //new web3.eth.Contract(EXCHANGE_ABI, fromToken[inputCurrency])
            //console.log("got c", c);
            c.tokenToTrxSwapInput(
        ...
        case 'TOKEN_TO_TOKEN':
          console.log(exchanges[window.tronWeb.address.fromHex(outputCurrency)])
          web3.contract().at(fromToken[inputCurrency])
            .then( c => {
            c.tokenToTokenSwapInput(
              BN(inputValue).multipliedBy(10 ** inputDecimals).toFixed(0),
              BN(outputValue).multipliedBy(10 ** outputDecimals).multipliedBy(1 - TOKEN_ALLOWED_SLIPPAGE).toFixed(0),
              '1',
              deadline*1000,
              outputCurrency,
              exchanges[window.tronWeb.address.fromHex(outputCurrency)]
