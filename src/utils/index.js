...
export function getEtherscanLink(networkId, data, type) {
  const prefix = `https://${ETHERSCAN_PREFIXES[networkId] || ETHERSCAN_PREFIXES[1]}tronscan.io`
  ...
  switch (type) {
    ...
    case 'address':
    default: {
      return `${prefix}/#/address/${TronWeb.address.fromHex(`41${data.slice(2)}`)}`
    }
  }
}
...
export function getAllQueryParams() {
  ...
  params.outputCurrency = isAddress(getQueryParam(window.location, 'outputCurrency'))
    ...
    : getQueryParam(window.location, 'outputCurrency') === 'TRX'
    ? 'TRX'
    : ''
...
export function shortenAddress(address, digits = 4) {
  if (!address.startsWith('T') && !isAddress(address)) {
...
export function getContract(address, ABI, library, account) {
  ...
  if (contractAddress.startsWith('0x')) {
    contractAddress = `41${address.slice(2)}`
  }
  /*
  if (address == '') {
    console.warn('getContract called with empty string address')
  ...
  */
  ...
  try {
    ...
    console.error('Error while instantiating tronweb contract', { abi, contractAddress })
    throw err
  }
}
...
export function formatEthBalance(balance) {
  // tron
  return amountFormatter(balance, 6, 6)
}
